...
class AdminCore
{
	...
	function CreateNewCatForm()
	{
		...
		$s = "Select `c_value` from ". $c["sql_struct_tab"] ." where `c_id`='templates_path'";
	   	$TemplatesDir = $sql->query($s,true);
			...
			//ekwo upd
			$out .= "<form><fieldset><legend>Создание новой страницы сайта</legend><label for='n_cat_link'>Краткое имя:</label>";
		...
		$out .= "<label for='n_cat_adm_title'>Заголовок:</label>";
		$out .= "<input type=\"text\" id=\"n_cat_adm_title\" name='n_cat_adm_title'/><div class='clear'></div>";
		
		$out .= "<label for='n_cat_temp'>Шаблон:</label>";
		$out .= "<input type=\"text\" title=\"При указаниии неизвестного шаблона - он будет создан\" id=\"n_cat_temp\" name='n_cat_temp' value=\"test.tpl\"/><div class='clear'></div>";
  ...
  if (this.title!='')
  {
	...
	function inter_CreateLeft( $result )
		...
		/*------------------------- Создать новую страницу/раздел сайта -----------------------------------*/
		
		if($_SESSION['access'] == 'Admin')
  		{
			$l ="'javascript: void(0)' onclick='xajax_CreateNewCatForm();'";
			$outCreatePage .= "<a class='new_page' href=$l>";
			$outCreatePage .= "Создать новый раздел";
			$outCreatePage .= "</a>";		
		}
//eo ekwo UPD
		/*------------------------- КОНЕЦ создать новую страницу/раздел сайта -----------------------------*/	
		$_i=0;
...
/*-----------------ВАЖНО!!!!  : папка с шаблонами !!!! ВСЕГДА в корневом каталоге сайта !!!! -----------------------*/
			$link = "../" . $link;
			...
			/*------------ СОЗДАНИЕ ВЫЕЗДЖАЮЩЕГО МЕНЮ ПОД КАТКГОРИЕЙ САЙТА -----------------------*/
        ...
        $out.="<a href='javascript: void(0);' onclick='$(\"#alias_add_{$page}\").toggle();'>Добавить алиас</a><div style='display: none;' id='alias_add_{$page}'><label for='alias_add_input_{$page}'>Добавить алиас:</label><input type='text' value='' name='alias_add_input_{$page}' id='alias_add_input_{$page}'><br />";
				$out.="<a href='javascript: void(0);' onclick='xajax_addAlias(\"$page\",$(\"#alias_add_input_{$page}\").val());'>Добавить</a></div>";
				//$out.=print_r($ret1);
				//eo ekwo UPD
				$out .= "Раздел:<br>";
				if( $ret1['v1']  ){
						...
						$out .=  "<input type=\"radio\" name=\"art_".$ret1['id']."\" value=\"0\" $l> Скрыт<Br>"; 
						$l = "onclick='xajax_SetVisibleCat(".$ret1['id'].",1)'";
						$out .= "<input type=\"radio\" name=\"art_".$ret1['id']."\" checked=\"checked\" value=\"1\" $l>Видим<Br>";
						
				}else{
						$l = "onclick='xajax_SetVisibleCat(".$ret1['id'].",0)'";
						$out .= "<input type=\"radio\" name=\"art_".$ret1['id']."\" checked=\"checked\" value=\"0\" $l>Скрыт<Br>";  
						$l = "onclick='xajax_SetVisibleCat(".$ret1['id'].",1)'";
						$out .= "<input type=\"radio\" name=\"art_".$ret1['id']."\" value=\"1\" $l> Видим<Br>";
				}
				/*---------------------------КНОПКА УДАЛЕНИЯ РАЗДЕЛА ---------------------------------------*/
				
				$l = "'javascript: void(0)' onclick='xajax_DeleteCategory({$row["id"]})'";
				$out .= "<a href = $l> Удалить раздел</a>";
  ...
  function checkForUpdates()
  {
    $eng = new Engine;	
		...
		if(floatval($version)<floatval($text))$out='<a onclick="xajax_update(\''.$text.'\');">Доступна новая версия '.$text.'</a>';
		return $out;
  }
  function update($par)
  {
...
/*
Удалить категорию на сайте 
Удалет из бызы struct  
*/
	...
	function inter_CreateTop( $result )
	{
        ...
        */
		...
		$out .= "<div><a href = $link>Мой&nbsp;профиль</a>&nbsp;<a href = exit.php>Выйти</a></div>";
		...
		$l="'javascript: void(0)' onclick='xajax_kern_DispModuleList();'";
			$out .= "<a class='rounded {transparent} button main2' style='display: none' href=$l>Список модулей</a>";
...
/***************************************************************************************************************/
        /*Модули привилегированного режима или меню ядра админки*/
...
/***************************************************************************************************************/        
		...
		if( !$MOD_PREV ) /*---------------------   Вызов обычного модуля  --------------*/ 
		{		
/*---------------------Вызывем админки в зависимости от шаблона--------------------*/
/*--------------------- Вызываем только тот  модуль который посылает себе команду --------------------*/
			if(isset($u_args['obj']))
			{
				$t = $m_data[ $u_args['obj'] ];
				unset($m_data);
				...
				if(isset($m_data)){
					...
					foreach($m_data AS $key => $val)
					{
						...
						//print $m_data["$mod"]['R_E_P']['count'];
							...
							{
								/*Предлагаем выбор редактируемых данных если один модуль выз. неск. раз*/
...
/*-------------------------------------------- КОМАНДЫ ГЛАВНОЙ АДМИНКИ ---------------------------------------------------*/
	...
	// записывает в свалку данные для нового модуля
	// пока только таблицы!!!
	   ...
	   $h   = opendir("../modules/");
		...
		while ($file = readdir($h))
		{
				...
				if( preg_match("/mod/",$file) )
				{
					...
					// подключаем главный файл модуля
					include("../modules/$file/admin/admin_$arr[1].php");
								
					// добавляем данные модуля в массив
					$data = $arr[1] . "_INFO";
					$data = $$data;
    ...
    $xajax->registerFunction(array("addAlias","AdminCore","addAlias"));
	  $xajax->registerFunction(array("removeAlias","AdminCore","removeAlias"));
	   	...
	   	// Функции связанные с правами модулей 
...
// заносит в бд информацию о правах разных пользователей для модулей
	 // а также считывает информацию из бд 
	function ModsAccessToDb(&$data,&$new_mods)
	{
		...
		foreach($data as $val)
		{
			...
			// если да, то считаем если нет, то запишем
			...
			if( empty($res) )
			{
				// добавляем новый модуль для инсталла 
				$new_mods[] = $val['name'];  
				
				
				// добавляем в бд инфо о настроиках доступа для модуля
				$s = "INSERT INTO `$tab` (`type`, `v1`, `v2`,`v3`,`v4`,`v5`,`v6`,`v7` )
			...
			{// записывем в наш масив данные из бд
				...
				// запишем в массив данные о используемых таблицах...
				...
				// запишем из базы данные о используемых VARS
				
				$s = "SELECT * FROM $tab where `type`='_value_' and `v1`='{$val['name']}'";
				$r = $sql->query($s,true);
				
				if(!empty( $r )) // если есть поле value для модуля...
				{
					$name 	= explode(",",$r['v2']);
					$title 	= explode(",",$r['v3']); 
					$vars 	= explode(",",$r['v4']);
   ...
   function ParseChiliTag($str,$tpl_args)
	...
	{	
		...
		/*Создает панели с нужными ид для функций-админок*/
	/*Создает панели с нужными ид для функций-админок*/
	...
	{	
		...
		for($i=0;$i<$n;$i++)
        {
			...
			/*------------- HE ПОД АДМИНИСТРАТОРОМ ОTКЛЮЧАЕМ РЕДАКТИРОВАНИЕ ТЕКСТА В ШАБЛОНЕ -------*/
